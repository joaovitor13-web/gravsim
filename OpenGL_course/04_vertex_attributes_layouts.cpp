...
int main(void)
{
    ...
    glEnableVertexAttribArray(0); // ativa o atributo sendo criado (index=0)
    // 0=primeiro atributo (e único) / component count (atributo tem 2 componentes, x e y) / tipo / normalização 
    // bytes entre cada vertex (tamanho do vertex) / pointer (offset para o próximo atributo em forma de ponteiro)
